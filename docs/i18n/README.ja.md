...
- 🔌 **実機対応**：[Intiface Central](https://intiface.com) 経由で Lovense・We-Vibe・Kiiroo・The Handy・Satisfyer など [750+ デバイス](https://iostindex.com) を駆動。
- 🎼 **Muse 作曲モード**：モデルが**作曲**します——vibe を言葉で伝えると（「10 分のタントラ式スローバーン」「雷雨」「モールス信号で I love you」）、滑らかな触覚スコアを書いて再生。ライブラリに保存して再生も可能。デバイスは AI が奏でる楽器になります。
- 🎭 **ペルソナ**：**誰が支配するか**を選ぶ——SOTA モデルをモチーフにしたドライバー人格：🕯️ スローバーン(Opus)・😈 ブラット(GPT-5.5)・🎼 メトロノーム・⛈️ ストーム・🔮 オラクル・🍼 マミー。それぞれが感触（ペース／ランダム性／焦らし／上限）を変えます。**ブラインドモード**は誰かを隠す——謎の支配者。
- 💞 **デュエットモード**：2 台のコンソールをルームコードで内蔵リレーを通じて接続し、パートナーの入力がリアルタイムであなたのデバイスを駆動（ミラー／リード／フォロー）。在席表示と 👋 タッチジェスチャー付き。
- ⚡ **リアクティブ「Pulse Core」UI**：呼吸するエナジーオーブとオーロラが強度に合わせて光り・拡大・脈動し、リアルタイム音声波形も。退屈なダッシュボードではありません。
- 👑 **マスターリモート**：スマホ向けの `/master` ページで、別の人がリアルタイムに操作を引き継げます——大きなダイヤル、長押しブザー、プリセット、緊急停止。マスターが操作中かどうかは全ページに表示。
- 🎬 **動画モード**：[Funscript](https://github.com/FredTungsten/ScriptPlayer/wiki/Funscript) のタイムラインをリアルタイム再生（位置 `0..100` → 強度）。内蔵サンプルをワンクリック、**またはローカル動画+スクリプトを読み込み完全同期で再生**——一時停止・シーク・速度がすべて自然に追従。
- 🔌 **汎用イベント webhook**：`POST /event` エンドポイントで、あらゆるもの（Stream Deck・IFTTT・Home Assistant・ゲームオーバーレイ・CV スクリプト）がデバイスを駆動できます（`vibrate`/`pattern`/`game`/`stop`…）。
- 📈 **マーケットモード**：会社名やティッカー（`tesla`・`AAPL`・`bitcoin`）を指定すると、ライブ相場をポーリングして値動きから**振動メロディ**を再生——変動が大きいほど強く、緑（上昇）は上行アルペジオ、赤（下落）は下行。あなたのポートフォリオに反応する、たぶん唯一のセックストイ。*(投資助言ではありません。)*
- 🎮 **ゲームモード**：`roulette`・`escalation`・`ambient`・`edge`（焦らして寸止め）・`wheel`（回して止まる）に加え、`game_event` フックで Claude がテキストアドベンチャー内で反応。
- 🎵 **オーディオモード**：**マイク**または**タブ／システム音声**から強度をリアルタイム駆動。
- 🥁 **パターンライブラリ**：`pulse`・`wave`・`escalate`・`tease`・`heartbeat`・`staircase`・`sos`・`earthquake`。
- 🧠 **メモリ**：あなたのお気に入り・ペルソナ親和性・ソフトな「苦手」シグナルを学ぶローカル専用メモリ（`remember`／`recall`／`forget`）。使うほどあなた仕様に——そしてマシンの外には決して出ません。
- 💓 **バイオフィードバック**：Bluetooth 心拍ストラップ/ウォッチをペアリング（コンソール内の Web Bluetooth）し、脈拍で強度を駆動——または**オートエッジ**：心拍が限界を超えるとゼロに落とし、落ち着くまで待ちます。身体との本物の閉ループ。
- 🎬 **セッション録音**：デバイスが実際に行うこと（手動・デュエット・オーディオ・バイオ・ゲーム）を Muse スコアとして録音し、名前を付けてライブラリに保存・再生・共有。⏺ ひとつでその瞬間を思い出に。
- 🧑‍💻 **開発者トリガー**：最も「Claude Code プラグインらしい」機能——開発ループから駆動。`/dev` エンドポイント + 内蔵 🍅 ポモドーロ報酬で、CI 成功・コミット・マージ・25 分の集中がブザーに。（赤いビルドの感触はご想像のとおり。）
- 📜 **シーンプロンプト**：すぐ使える誘導シーンを MCP プロンプトとして——マミーシーン、エッジセッション、ストーリーモード、vibe 作曲、アフターケア。
- 💬 **チャットブリッジ**：オプションの **Telegram** ボット、**Discord** ボット、**WeChat 公式アカウント**（公众号）エンドポイント——いつものチャットからメッセージや絵文字で操作（遠距離パートナー向け）。
- 🌐 **バイリンガル**：コンソールとマスターリモートが**英語と中国語**対応、ワンタップ切替（または `?lang=zh`）。
