...
<sub>▶️ <b>功能速览</b> — 所有玩法一次看完（约 25 秒）。</sub>
...
## ▶️ 用 Claude Code、Codex 或任意 MCP 客户端使用

claude-f-me 是标准的 **MCP 服务**——可从 **Claude Code、Codex、Cursor、Cline、Claude Desktop** 或任何支持 MCP 的工具驱动。**无需硬件**，内置模拟器跑完整体验（地址 **http://localhost:8731**）。现成配置见 [`examples/`](./examples)。

**🟣 Claude Code** —— 作为插件一键安装（含 slash 命令）：
...
```jsonc
// "mcpServers" 条目（Claude Desktop / Cursor / Cline / Windsurf）—— 见 examples/mcp.json
"claude-f-me": {
  "command": "npx",
...
Codex 用 TOML —— 把 [`examples/codex-config.toml`](./examples/codex-config.toml) 放进 `~/.codex/config.toml`。各客户端对照表见 [`examples/`](./examples)。

然后到处都照样对话——`扫描设备` · `开始 edge 游戏` · `作一段慢曲`——或在 Claude Code 里用 slash 命令 `/claude-f-me:fuck`、`:edge`、`:morse`、`:safeword`。
...
**🎛️ 驱动设备**
- 🎼 **Muse 作曲**——描述一个 vibe（"一场雷暴"、"用摩斯电码说我爱你"），模型作出平滑触感曲线并播放；可存、可重播。
- 🥁 **节奏**——`脉冲` · `波浪` · `递增` · `挑逗` · `心跳` · `楼梯` · `SOS` · `地震`。
- 🎮 **游戏**——`轮盘` · `递增` · `环境` · `边缘`（挑逗-拒绝）· `转盘`，外加 `game_event` 钩子供文字冒险用。
- 🎵 **音频**——用**麦克风**或**标签页/系统声音**实时驱动强度。

**🎭 由谁掌控**
- 🎭 **人格**——选谁来开车（🕯️ 慢炖/Opus · 😈 小恶魔/GPT-5.5 · 🎼 节拍器 · ⛈️ 风暴 · 🔮 神谕 · 🍼 妈咪），各自改变手感，**盲选模式**藏起是谁。
- 👑 **主人遥控**——把 `/master` 页面交给别人实时接管（旋钮、按住震动、预设、急停）。
- 💞 **双人**——两台控制台经中继连起来，伴侣实时驱动你的设备（镜像/主导/跟随），带 👋 触碰。

**🌍 真实世界输入**
- 🎬 **视频**——播放 [Funscript](https://github.com/FredTungsten/ScriptPlayer/wiki/Funscript)，或本地视频+脚本完美同步。
- 📈 **市值**——报上代码（`特斯拉`、`比特币`），把实时涨跌变成震动旋律。*(非投资建议。)*
- 💓 **生物反馈**——蓝牙心率带驱动强度，或**自动边缘**：心率冲过临界就断电。
- 🔌 **事件 webhook**——`POST /event`，接 Stream Deck、IFTTT、Home Assistant、游戏 overlay、CV 脚本…
- 🧑‍💻 **开发者触发**——commit、CI 通过、合并或 🍅 番茄钟都能经 `/dev` 让它响。
- 💬 **聊天桥接**——用消息或 emoji 从 **Telegram**、**Discord**、**微信公众号**控制。

**🎨 个性化**
- ⚡ **Pulse Core 界面**——呼吸的能量球 + 极光随强度脉动，外加实时音波——不是无聊面板。
- 🧠 **记忆**——仅本地；学习你的偏好、人格亲和度与软不喜欢（`remember`/`recall`/`forget`），绝不外传。
- 🎬 **会话录制**——把设备做过的一切（手动/双人/音频/生物/游戏）录成可重播的 Muse 曲子。
- 📜 **场景提示词**——引导场景做成 MCP prompts（妈咪、边缘、剧情、作曲、事后温存）。
- 🌐 **双语**——控制台与遥控页支持**中英文**（`?lang=zh`）。

**🔌 硬件与安全**
- 🔌 **真实硬件**——经 [Intiface](https://intiface.com) 驱动 Lovense、We-Vibe、Kiiroo、The Handy、Satisfyer 等 [750+ 款设备](https://iostindex.com)。
