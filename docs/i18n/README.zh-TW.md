...
- 🎭 **人格**：選擇**由誰掌控**——按 SOTA 模型原型設計的驅動人格：🕯️ 慢燉(Opus)、😈 小惡魔(GPT-5.5)、🎼 節拍器、⛈️ 風暴、🔮 神諭、🍼 媽咪。各自改變手感（節奏/混亂度/拒絕傾向/上限）。**盲選模式**藏起是誰——神秘掌控者。
- 💞 **雙人模式**：兩台主控台用房間碼透過內建中繼連起來，伴侶的輸入即時驅動你的裝置（鏡像/主導/跟隨），帶在線狀態和 👋 觸碰手勢。
- ⚡ **「脈動核心」反應式介面**：呼吸發光的能量球 + 極光背景隨強度即時放大/發光，外加即時音波。
- 👑 **主人遙控**：手機友善的 `/master` 頁面，讓另一個人即時接管控制——大旋鈕、按住震動、預設、急停。每個頁面都會顯示有幾位主人在控制。
- 🎬 **影片模式**：即時播放 [Funscript](https://github.com/FredTungsten/ScriptPlayer/wiki/Funscript) 時間軸（位置 `0..100` → 強度）。內建範例一鍵試玩，**或選本機影片+腳本，邊看邊完美同步**——暫停/拖動/倍速都自動跟上。
- 🔌 **通用事件 webhook**：`POST /event` 端點，讓任何東西（Stream Deck、IFTTT、Home Assistant、遊戲 overlay、CV 腳本）都能驅動裝置（`vibrate`/`pattern`/`game`/`stop`…）。
- 📈 **市值模式**：報上公司名或代碼（`特斯拉`、`AAPL`、`比特幣`），它拉取即時行情，把漲跌變成**震動旋律**——波動越大越強，綠漲=上行琶音、紅跌=下行。大概是唯一一個會對你持倉做出反應的情趣玩具。*(非投資建議。)*
- 🎮 **遊戲模式**：`輪盤`、`遞增`、`環境`、`邊緣`（挑逗-拒絕）、`轉盤`（旋轉停留），還有 `game_event` 鉤子讓 Claude 在文字冒險裡即時反應。
- 🎵 **音訊模式**：用**麥克風**或**分頁/系統聲音**即時驅動強度。
- 🥁 **節奏庫**：`脈衝`、`波浪`、`遞增`、`挑逗`、`心跳`、`樓梯`、`SOS`、`地震`。
- 🧠 **記憶**：本機記憶，學習你的偏好、人格親和度和「軟不喜歡」訊號（`remember`/`recall`/`forget`），越用越懂你——且絕不外傳。
- 💓 **生物回饋**：在主控台用 Web Bluetooth 直連藍牙心率帶/手錶，讓心率驅動強度——或**自動邊緣**：心率衝過臨界就斷電，等你平復再續。和身體的真閉環。
- 🎬 **工作階段錄製**：把裝置實際發生的一切（手動、雙人、音訊、生物回饋、遊戲）錄成一條 Muse 曲子，命名後存進曲庫可重播、可分享。一個 ⏺ 把此刻變成收藏。
- 🧑‍💻 **開發者觸發**：最「Claude Code 外掛」的功能——從你的開發流驅動它。`/dev` 端點 + 內建 🍅 番茄鐘獎勵：CI 通過、commit、合併或專注滿 25 分鐘都能讓它響。（紅色建置什麼手感你自己想。）
- 📜 **場景提示詞**：現成的引導場景，做成 MCP prompts——媽咪場景、邊緣控制、劇情模式、作曲一個 vibe、事後溫存。
- 💬 **聊天橋接**：可選 **Telegram** 機器人、**Discord** 機器人和**微信公眾號**端點——在你天天用的聊天軟體裡用訊息或 emoji 控制（異地伴侶神器）。
